...
static void
GT_SetString(Object *ra_obj, STRPTR *string)
{
	STRPTR t;

	if( GetAttrs(ra_obj, STRINGA_TextVal,&t, TAG_END) )
	{
		FreeString(*string);
...
{
	uint16 wcode = 0;
	uint32 wresult = WMHI_LASTMSG;
	STRPTR res_string;
	struct Mappe *mp = wd->wd_Data;

	while( (wresult=IDoMethod(winobj[WDT_DOCINFO], WM_HANDLEINPUT, &wcode)) != WMHI_LASTMSG )
	{
DBUG("DOCINFO wresult=0x%08lx (wcode=0x%08lx)\n", wresult,wcode);
		switch(wresult & WMHI_CLASSMASK)
		{
			case WMHI_MENUPICK:
DBUG("DOCINFO WMHI_MENUPICK (wcode=0x%08lx) (imsg.Code=0x%08lx)\n", wcode,imsg.Code);
				imsg.Code = wcode; // so HandleMenu() "knows" menu option
				HandleMenu();
			break;

			case WMHI_CLOSEWINDOW:
DBUG("\tDOCINFO WMHI_CLOSEWINDOW\n");
				CloseAppWindow(win, TRUE);
			break;

			case WMHI_GADGETUP:
DBUG("DOCINFO WMHI_GADGETUP (0x%08lx)\n", (wresult&WMHI_CLASSMASK));
				switch(wresult & WMHI_GADGETMASK)
				{
					case OID_OK://9
DBUG("\tDOCINFO OID_OK\n");
						GT_SetString(OBJ(OID_DI_AUTHOR), &mp->mp_Author);
						GT_SetString(OBJ(OID_DI_VERSION), &mp->mp_Version);
						GT_SetString(OBJ(OID_DI_KEYWORDS), &mp->mp_CatchWords);

						res_string = (STRPTR)DoGadgetMethod(GAD(OID_DI_COMMENTS), win, NULL, GM_TEXTEDITOR_ExportText, NULL);
						if(res_string != NULL)
						{
							FreeString(mp->mp_Note);
							mp->mp_Note = AllocString(res_string);
						}
					//break;
					case OID_CANCEL://11
DBUG("\tDOCINFO OID_CANCEL\n");
						CloseAppWindow(win, TRUE);
					break;
...
DBUG("DOCINFO HandleDocInfoIDCMP() END\n\n");
}
...
closeNotesWin(REG(a0, struct Window *win), REG(d0, BOOL clean))
{
	...
	if (clean)
	{
		while((wn = (struct wdtNote *)MyRemHead((struct List *)wd->wd_ExtData[0])) != 0)
		{
			...
			//RA
		// Clean/Free public notes list
		struct Node *nextnode, *node = GetHead( (struct List *)wd->wd_ExtData[1] );
		while(node)
		{
DBUG("closeNotesWin(): Free'ing notes (0x%08lx) from list (0x%08lx)\n", node,wd->wd_ExtData[1]);
...
handleNotesIDCMP(REG(a0, struct TagItem *tag))
{
	...
	while( (wresult=IDoMethod(winobj[WDT_NOTES], WM_HANDLEINPUT, &wcode)) != WMHI_LASTMSG )
	{
DBUG("CREATENOTES wresult=0x%08lx (wcode=0x%08lx)\n", wresult,wcode);
		switch(wresult & WMHI_CLASSMASK)
		{
			case WMHI_MENUPICK:
DBUG("CREATENOTES WMHI_MENUPICK (wcode=0x%08lx) (imsg.Code=0x%08lx)\n", wcode,imsg.Code);
				imsg.Code = wcode; // so HandleMenu() "knows" menu option
				HandleMenu();
			break;

			case WMHI_CLOSEWINDOW:
DBUG("\tCREATENOTES WMHI_CLOSEWINDOW\n");
				CloseAppWindow(win, TRUE);
			break;

			case WMHI_GADGETUP:
DBUG("CREATENOTES WMHI_GADGETUP (0x%08lx)\n", (wresult&WMHI_CLASSMASK));
				switch(wresult & WMHI_GADGETMASK)
				{
					case OID_CN_INSERT:
DBUG("\tCREATENOTES OID_CN_INSERT (%ld)\n",wcode);
					{
						struct wdtNote *wn;
						STRPTR t;
						long   len;

						wn = (struct wdtNote *)FindListNumber(wd->wd_ExtData[0], wcode); // wcode -> index item choosed
DBUG("wn->wn_Note='%s'\n",wn->wn_Note);
						res_string = (STRPTR)DoGadgetMethod(GAD(OID_CN_LISTNOTES), win, NULL, GM_TEXTEDITOR_ExportText, TAG_END);

						len = strlen(res_string) + strlen(wn->wn_Note) + 1;
						if( res_string && wn->wn_Note && (t=AllocPooled(pool,len)) )
						{
DBUG("\t1 res_string ='%s'\n",res_string);
DBUG("\t1 wn->wn_Note='%s'\n",wn->wn_Note);
							strcpy(t, res_string);
							strcat(t, wn->wn_Note);
DBUG("\t1 '%s'\n",t);
							SetGadgetAttrs(GAD(OID_CN_LISTNOTES), win, NULL, GA_TEXTEDITOR_Contents,t, TAG_DONE);
							FreePooled(pool, t, len);
						}
						else
						{
DBUG("\t2 '%s'\n",wn->wn_Note);
							SetGadgetAttrs(GAD(OID_CN_LISTNOTES), win, NULL, GA_TEXTEDITOR_Contents,wn->wn_Note, TAG_DONE);
						}
					}
					break;
					case OID_OK://9
DBUG("\tCREATENOTES OID_OK\n");
						res_string = (STRPTR)DoGadgetMethod(GAD(OID_CN_LISTNOTES), win, NULL, GM_TEXTEDITOR_ExportText, TAG_END);
						if( res_string && (i=strlen(res_string)) )
						{
							strcpy(t, GetString(&gLocaleInfo,MSG_ATTACH_NOTE_UNDO));
							strcat(t, "\"");
							strncat(t, res_string, 222);
							if(i > 222)
							{
								strcat(t, "...");
							}
...
DBUG("\t'%s'\n",t);
						...
						while( (tf=GetMarkedFields(rxpage, tf, TRUE)) != 0 )
						{
							...
							tf->tf_Note = AllocString(res_string);
						}
						EndUndo(rxpage);
...
DBUG("CREATENOTES handleNotesIDCMP() END\n\n");
}
