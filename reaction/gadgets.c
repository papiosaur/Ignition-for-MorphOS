...
// [AUTHOR|VERSION|KEYWORDS]
			LAYOUT_AddChild, NewObject(LayoutClass, NULL, //"layout.gadget",
				...
				//LAYOUT_BevelStyle,  BVS_GROUP,
				LAYOUT_AddChild, OBJ(OID_DI_AUTHOR) = NewObject(StringClass, NULL, //"string.gadget",
					GA_ID,        0,//OID_DI_AUTHOR,
					GA_RelVerify, TRUE,
					GA_TabCycle,  TRUE,
					//GA_HintInfo,  "",
					STRINGA_TextVal, mp->mp_Author,
				TAG_DONE),
				CHILD_Label, NewObject(LabelClass, NULL, //"label.image",
					LABEL_Text, GetString(&gLocaleInfo,MSG_AUTHOR_ULABEL),
				TAG_DONE),
				LAYOUT_AddChild, OBJ(OID_DI_VERSION) = NewObject(StringClass, NULL, //"string.gadget",
					GA_ID,        0,//OID_DI_VERSION,
					GA_RelVerify, TRUE,
					GA_TabCycle,  TRUE,
					//GA_HintInfo,  "",
					STRINGA_TextVal, mp->mp_Version,
				TAG_DONE),
				CHILD_Label, NewObject(LabelClass, NULL, //"label.image",
					LABEL_Text, GetString(&gLocaleInfo,MSG_VERSION_ULABEL),
				TAG_DONE),
				LAYOUT_AddChild, OBJ(OID_DI_KEYWORDS) = NewObject(StringClass, NULL, //"string.gadget",
					GA_ID,        0,//OID_DI_KEYWORDS,
					GA_RelVerify, TRUE,
					GA_TabCycle,  TRUE,
					//GA_HintInfo,  "",
					STRINGA_TextVal, mp->mp_CatchWords,
				TAG_DONE),
				CHILD_Label, NewObject(LabelClass, NULL, //"label.image",
					LABEL_Text, GetString(&gLocaleInfo,MSG_KEYWORDS_ULABEL),
				TAG_DONE),
			TAG_DONE), // END [AUTHOR|VERSION|KEYWORDS]
			CHILD_WeightedHeight, 0,
// [COMMENT]
			LAYOUT_AddChild, NewObject(LayoutClass, NULL, //"layout.gadget",
				LAYOUT_Orientation, LAYOUT_ORIENT_HORIZ,
				//LAYOUT_BevelStyle,  BVS_GROUP,
				LAYOUT_AddChild, OBJ(OID_DI_COMMENTS) = NewObject(TexteditorClass, NULL, //"texteditor.gadget",
					GA_ID,       0,//OID_DI_COMMENTS,
					GA_TabCycle, TRUE,
					//GA_TEXTEDITOR_ReadOnly, TRUE,
					GA_TEXTEDITOR_Contents, mp->mp_Note,
					//GA_TEXTEDITOR_Length,   120,
				TAG_DONE),
				CHILD_MinHeight, (fontheight*5)+10,
//				CHILD_MinWidth,  4*TLn(GetString(&gLocaleInfo,MSG_COMMENTS_LABEL)),//350,
				CHILD_Label, NewObject(LabelClass, NULL, //"label.image",
					LABEL_Text, GetString(&gLocaleInfo,MSG_COMMENTS_ULABEL),
				TAG_DONE),
				LAYOUT_AddChild, OBJ(OID_DI_VSCROLL) = NewObject(ScrollerClass, NULL, //"scroller.gadget",
					//GA_ID, 0,//OID_DI_VSCROLL,
					//SCROLLER_Orientation, SORIENT_VERT,
					SCROLLER_Arrows, FALSE,
				TAG_DONE),
			TAG_DONE), // END [COMMENT]
			CHILD_MinWidth, 4*TLn(GetString(&gLocaleInfo,MSG_COMMENTS_LABEL)),//350,
// [BUTTONS]
			LAYOUT_AddChild, NewObject(LayoutClass, NULL, //"layout.gadget",
				LAYOUT_Orientation, LAYOUT_ORIENT_HORIZ,
			...
			CHILD_WeightedHeight, 0,
	TAG_END);
DBUG("CreateDocInfoGadgets() obj=0x%08lx\n", OBJ(OID_TEMPORAL));
	// "Bind" scroller to texteditor window
	SetAttrs(OBJ(OID_DI_COMMENTS), GA_TEXTEDITOR_VertScroller,OBJ(OID_DI_VSCROLL), TAG_DONE);
}
...
{
	STRPTR res_string = NULL;
//wd->wd_ExtData[0] -> notes list [NATIVE]
//wd->wd_ExtData[1] -> notes chooser list [REACTION] (was texteditor gadget)
DBUG("CreateNotesGadgets(): wid=%ld(%ld)   hei=%ld\n",wid,(fontheight*20),hei);
	if(rxpage && rxpage->pg_Gad.tf)
	{
		res_string = rxpage->pg_Gad.tf->tf_Note;
	}
...
// [NOTE]
			LAYOUT_AddChild, NewObject(LayoutClass, NULL, //"layout.gadget",
				LAYOUT_Orientation, LAYOUT_ORIENT_HORIZ,
				//LAYOUT_BevelStyle,  BVS_GROUP,
				LAYOUT_AddChild, OBJ(OID_CN_LISTNOTES) = NewObject(TexteditorClass, NULL, //"texteditor.gadget",
					GA_ID,       0,//OID_CN_LISTNOTES,
					GA_TabCycle, TRUE,
					//GA_TEXTEDITOR_ReadOnly, TRUE,
					GA_TEXTEDITOR_Contents, res_string,
					//GA_TEXTEDITOR_Length,   120,
				TAG_DONE),
				CHILD_MinHeight, (fontheight*5)+10,
//				CHILD_MinWidth,  (fontheight*20),//wid,
				LAYOUT_AddChild, OBJ(OID_CN_VSCROLL) = NewObject(ScrollerClass, NULL, //"scroller.gadget",
					//GA_ID, 0,//OID_CN_VSCROLL,
					//SCROLLER_Orientation, SORIENT_VERT,
					SCROLLER_Arrows, FALSE,
				TAG_DONE),
			TAG_DONE), // END [NOTE]
			...
			LAYOUT_AddChild, NewObject(LayoutClass, NULL, //"layout.gadget",
				...
				LAYOUT_AddChild, OBJ(OID_CN_INSERT) = NewObject(ChooserClass, NULL, //"chooser.gadget",
					GA_ID,         OID_CN_INSERT,
					GA_RelVerify,  TRUE,
					//GA_HintInfo,   "",
					GA_Underscore, 0,
					CHOOSER_DropDown, TRUE,
					CHOOSER_Labels,   wd->wd_ExtData[1], // "shortened" notes list
					//CHOOSER_Selected, 0,
				TAG_DONE),
				CHILD_Label, NewObject(LabelClass, NULL, //"label.image",
					LABEL_Text, GetString(&gLocaleInfo,MSG_INSERT_EXISTING_NOTE_GAD),
				TAG_DONE),
			TAG_DONE),
			CHILD_WeightedHeight, 0,
// [BUTTONS]
			LAYOUT_AddChild, NewObject(LayoutClass, NULL, //"layout.gadget",
				...
				LAYOUT_AddChild, NewObject(ButtonClass, NULL, //"button.gadget",
					GA_ID,        OID_DELETE,
					GA_RelVerify, TRUE,
					GA_Text,      GetString(&gLocaleInfo,MSG_DELETE_GAD),//"Delete"
				TAG_DONE),
				CHILD_WeightedWidth, 0,
				LAYOUT_AddChild, NewObject(ButtonClass, NULL, //"button.gadget",
					GA_ID,        OID_CANCEL,//11
			...
			CHILD_WeightedHeight, 0,
	TAG_END);
DBUG("CreateNotesGadgets() obj=0x%08lx\n", OBJ(OID_TEMPORAL));
	// "Bind" scroller to texteditor window
	SetAttrs(OBJ(OID_CN_LISTNOTES), GA_TEXTEDITOR_VertScroller,OBJ(OID_CN_VSCROLL), TAG_DONE);
}
