...
// RA
void
CopyRALBAppKeys(struct MinList *from, struct List *to, struct MinList *old_to)
{
		...
		if( (ak=AllocPooled(pool,sizeof(struct AppKey))) != 0 )
		{
			*ak = *sak;
			...
			if( (node=AllocListBrowserNode(2,
			                               ...
			                               //LBNA_Column,1, LBNCA_Integer,(uint32 *)ak, // AppKey list [NATIVE] item
			                               LBNA_Column,1, LBNCA_CopyText,TRUE, LBNCA_Text,ak->ak_AppCmd,
			                               LBNA_UserData, (APTR)ak,
			                              TAG_DONE)) )
			{
				AddTail(to, node);
DBUG("CopyRALBAppKeys(): node(0x%08lx) '%s' '%s' ak=0x%08lx\n", node,ak->ak_Node.ln_Name,ak->ak_AppCmd,(uint32 *)ak);
			}
		}
	}
}
