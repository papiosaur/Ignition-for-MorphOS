...
#ifdef __amigaos4__
//RA
uint32 PopUpRAListA(struct ColumnInfo *ci, struct List *lst, struct TagItem *tags)
{
	uint32 pres_val, presult = WMHI_LASTMSG, psignal = 0, pdone = FALSE;
	uint16 pcode;
	int32  items, width, selected, horiz, space;
	struct Window *popWindow;
	struct Node *popup_n = NULL;
	...
	width = GetTagData(POPA_Width, 250, (struct TagItem *)tags);
	items = GetTagData(POPA_MaxItems, 20, (struct TagItem *)tags);
	horiz = GetTagData(POPA_HorizProp, 0, (struct TagItem *)tags);
	space = GetTagData(POPA_ItemHeight, 1, (struct TagItem *)tags); // used as "gap" between items
DBUG("selected=%ld   width=%ld   items=%ld   horiz=%ld\n",selected,width,items,horiz);
	Object *obj_pop = NewObject(WindowClass, NULL, //"window.class",
	                            WA_PubScreen,  scr,
	                            WA_Activate,   TRUE,
	                            //WA_Borderless, TRUE,
	                            //WA_Width,      width,
	                            ...
	                            WINDOW_Layout, NewObject(LayoutClass, NULL, //"layout.gadget",
	                              ...
	                              LAYOUT_AddChild, OBJ(OID_POPUP) = NewObject(ListBrowserClass, NULL, //"listbrowser.gadget",
	                               GA_ID,        OID_POPUP,
	                               GA_RelVerify, TRUE,
	                               LISTBROWSER_ColumnInfo,   ci,
	                               LISTBROWSER_ColumnTitles, FALSE,
	                               LISTBROWSER_AutoFit,      TRUE,
	                               LISTBROWSER_Labels,       lst,
	                               LISTBROWSER_MakeVisible,  selected,
	                               //LISTBROWSER_Selected,     selected,
	                               LISTBROWSER_ShowSelected, TRUE,
	                               //LISTBROWSER_Striping,       LBS_ROWS,
	                               //LISTBROWSER_Borderless,     TRUE,
	                               LISTBROWSER_MinVisible,     items,
	                               LISTBROWSER_HorizontalProp, horiz? TRUE:FALSE,
	                               LISTBROWSER_VertSeparators, FALSE,
	                               LISTBROWSER_Spacing,        space,
	                              TAG_DONE),
	                              CHILD_MinWidth, width, // in pixels
	                            TAG_DONE),
	                           TAG_END);
DBUG("obj_pop=0x%08lx\n",obj_pop);
	popWindow = (struct Window *)IDoMethod(obj_pop, WM_OPEN, NULL);
DBUG("popWindow=0x%08lx\n",popWindow);
	if(popWindow)
	{
		GetAttr(WINDOW_SigMask, obj_pop, &psignal);

		while(!pdone)
		{
			Wait(psignal);
			while( (presult=IDoMethod(obj_pop,WM_HANDLEINPUT,&pcode)) != WMHI_LASTMSG ) 
			{
				switch(presult & WMHI_CLASSMASK)
				{
					case WMHI_MENUPICK: // clicked RMB
					case WMHI_INACTIVE: // clicked outside PopUp window
						pdone = TRUE;
					break;
					case WMHI_GADGETUP:
DBUG("\tWMHI_GADGETUP (%08lx):\n",presult);
						switch(presult & WMHI_GADGETMASK)
						{
							case OID_POPUP:
DBUG("\tOID_POPUP %ld\n",pcode);
								GetAttr(LISTBROWSER_RelEvent, OBJ(OID_POPUP), &pres_val);
								if(pres_val == LBRE_DOUBLECLICK)
								{
									struct Node *res_n = NULL;

									GetAttr(LISTBROWSER_SelectedNode, OBJ(OID_POPUP), (uint32 *)&res_n);
									GetListBrowserNodeAttrs(res_n, LBNA_UserData,&popup_n, TAG_END);
DBUG("\t'%s' 0x%08lx\n",popup_n->ln_Name,popup_n);
									IDoMethod(obj_pop, WM_CLOSE, NULL);
									pdone = TRUE;
								}
							break;
						} // END switch(presult & WMHI_GADGETMASK)
					break;
				} // END switch(presult & WMHI_CLASSMASK)
			} // END while( (presult=IDoMethod(..
		}
	}

	DisposeObject(obj_pop);
	obj_pop = NULL;
	popWindow = NULL;
DBUG("\tOID_POPUP END 0x%08lx\n",popup_n);
	return( (uint32)popup_n );
}

uint32 VARARGS68K PopUpRAList(struct ColumnInfo *ci, struct List *lst, ...);
uint32 PopUpRAList(struct ColumnInfo *ci, struct List *lst, ...)
{
	va_list ap;
	struct TagItem *tags;

	va_startlinear(ap, lst);
	tags = va_getlinearva(ap, struct TagItem *);
	return( PopUpRAListA(ci,lst,tags) );
}
//RA
