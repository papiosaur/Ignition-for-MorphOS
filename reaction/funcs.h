...
// RA
#ifdef _DEBUGRA_
	#define DBUG(x,...) DebugPrintF("[IGNITION|%s:%-4ld] "x, __FILE__,__LINE__, ##__VA_ARGS__)
#else
	#define DBUG(x,...)
#endif
// RA
...
extern void CopyRALBAppKeys(struct MinList *from, struct List *to, struct MinList *old_to); // RALB = ReAction ListBrowser
...
extern void MakeLocaleStrings(struct MinList *list, LONG id, ...) VARARGS68K;
	...
	extern struct List *MakeLocaleStringRACList(LONG id, ...) VARARGS68K; // RAC = ReAction Chooser
...
extern void FreeListItems(struct MinList *list, APTR free);
	...
	extern void CopyRALBListItems(struct MinList *from, struct List *to, APTR copy, struct MinList *old); // RALB = ReAction ListBrowser
