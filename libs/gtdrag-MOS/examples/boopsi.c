...
void DrawSimpleGadget(struct RastPort *rp,struct Gadget *gad,struct GadgetInfo *gi,struct SimpleGData *sd,BOOL all)
{
  ...
  if (all)
  {
    ...
    if ((im =(struct Image *) NewObject(NULL,"frameiclass",IA_Width,     gad->Width,
...
ULONG DispatchSimpleGadget(REG(a0, Class *cl),REG(a2, Object *o),REG(a1, Msg msg))
{
...
//  if ((GTDragBase = OpenLibrary("gtdrag.library",3)) != 0)
  {
    if (GTD_AddApp("boopsitest",GTDA_NewStyle,TRUE,TAG_END))
    {
      if ((scr = LockPubScreen(NULL)) != 0)
      {
        ...
        {
          if ((hgad = (struct Gadget *)NewObject(simplegclass,NULL,GA_Left,       scr->WBorLeft,
                                                 ...
                                                 TAG_END)) != 0)
          {
            ...
            if ((vgad = (struct Gadget *)NewObject(simplegclass,NULL,GA_Left,       scr->WBorLeft,
                                                   ...
                                                   TAG_END)) != 0)
            {
              ...
              DisposeObject((Object *)vgad);
            }
            DisposeObject((Object *)hgad);
          }
          FreeVisualInfo(vi);
        }
        clean();
