...
struct MsgPort *iport, *rxport, *wbport, *notifyport, *tport;
void   *vi;
APTR   pool;
...
MakeIconGads(struct Prefs *pr,struct winData *wd,struct Gadget *gad,long w,long h)
{
    ...
    {
#ifdef __amigaos4__
        ngad.ng_Width = pr->pr_Icon->pi_Width+4;
        ngad.ng_Height = pr->pr_Icon->pi_Height+3;
#else
...
MakeProjectGadgets(struct winData *wd,long w,long h)
{
    ...
    if (pd->pd_FormBar)
    {
...
#ifdef __amigaos4__
        ngad.ng_Width = w-ngad.ng_LeftEdge-upImg->Width-82-2*boxwidth-TLn(GetString(&gLocaleInfo, MSG_CHOOSE_GAD))-TLn("Seite 999") - 151;
#else
        ...
        ngad.ng_LeftEdge += ngad.ng_Width+6;
#ifdef __amigaos4__
        ngad.ng_Width = TLn("Seite 999")+202;
#else
...
InitApp(void)
{
    ...
    SearchFonts();
#ifdef __amigaos4__
    if (!(stdfamily = MyFindName(&families, "DejaVu Sans")) && !IsListEmpty((struct List *)&families))
#else
...
HandleApp(void)
{
    ...
    while (!ende)
    {
		...
		/* Handle ARexx/ Timer  messages */
#ifdef __amigaos4__
		/*did not work stable, when rexx and timer shared a port, so they are seperated*/
		...
		while ((rxmsg = (struct RexxMsg *)GetMsg(tport)) != 0) {
			handleTimer();
        }
#else
		while ((rxmsg = (struct RexxMsg *)GetMsg(rxport)) != 0) {
    ...
    shelldir = SetCurrentDir(GetProgramDir());
	{
		...
		if (GTD_AddApp("ignition", GTDA_NewStyle, TRUE, TAG_END)) {
			...
			if ((iport = CreateMsgPort()) != 0) {
                ...
                InitApp();
#ifdef __amigaos4__
			    Strlcpy(timername, pubname, 60);
			    Strlcat(timername, ".TIMER", 60);
				if ((rxport = AllocSysObjectTags(ASOT_PORT, ASOPORT_Name, pubname, ASOPORT_Pri, 0, TAG_END)) != 0 && (tport = AllocSysObjectTags(ASOT_PORT, ASOPORT_Name, timername, ASOPORT_Pri, 0, TAG_END)) !=0) {
#else
				if ((rxport = CreatePort(pubname, 0)) != 0) {
#endif
#ifdef __amigaos4__
					if ((sTimeRequest = (struct timerequest *)AllocSysObjectTags(ASOT_IOREQUEST, ASOIOR_ReplyPort, tport, ASOIOR_Size, sizeof(struct timerequest), TAG_END)) != 0) {
#else
