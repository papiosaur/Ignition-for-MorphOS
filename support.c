...
#endif
	{
//DebugPrintF("[IGNITION|%s:%-4ld] FreeString():'%s' (%ld)\n",__FILE__,__LINE__, t,strlen(t));
...
#ifdef __amigaos4__
/*
Replaces a string in another. all 0 or only the occourence which is set by number.
Parameter	! Description
------------+-------------------------------------------------------------------------------------------------------
 sstr		! big string with the substring which will be replaced
 rstr		! string which must be replaced
 nstr		! new string which is replace the rstr
 pos		! 0 every string is replaced n, only the n occourance is replaced
----
Return:
*/
void ReplaceTextString(char *sstr, char *rstr, char *nstr, int8 pos)
{
    uint32 i1, ds;																					//indices
    uint32 count = 0;																				//
    char buffer[2048];																				//temp. buffer for result
	uint32 len1, len2;																				//save stringlength
	
	len1 = Strlen(sstr);																			//store the length
	len2 = Strlen(rstr);
    if((int32)(len1 - len2) < 0 )																	//no replace nessessary if string to short
    	return;
    for(i1 = 0, ds = 0; i1 < len1; i1++)															//search the rstr in sstr
    	if(!Strnicmp(&sstr[i1], rstr, len2))														//found the rstr in sstr
    	{						
    		count++;																				//increase occourance counter
    		if(count == pos || pos == 0)															//Found the wished string at the right number or all
    		{																							
	    	    Strlcpy(&buffer[ds], nstr, 2048 - ds);												//copy data from new text in buffer
    		    ds += Strlen(nstr);																	//inc destination string index
    		    i1 += len2 - 1;																		//inc index for the source
    		}
    		else																					//not the right position
    		{
    		    buffer[ds++] = sstr[i1];															//copy the first character of the wrong pos
    		}
    	    if(pos > 0 && count > pos )																//whished position reached
    	    {
    	        Strlcat(buffer, &sstr[i1 + 1], 2048);												//Rest des Src-String anhängen
    	        ds = Strlen(buffer);																//ds neu setzen
				break;																				//Dann abbrechen
			}
    	}
    	else
    	{
    	    buffer[ds++] = sstr[i1];																//not the searchstring just copy
    	}
    buffer[ds] = '\0';																				//set string end mark
    Strlcpy(sstr, buffer, 2048);																	//copy result in src
    return;
}
#endif
...
RemoveFromArrayList(struct ArrayList *al, void *entry)
{
	...
	{
	    DebugPrintF("RemoveFromArrayList: Pointer: 0x%08X Last:%8d Size:%8d\n", al, al->al_Last, al->al_Size);
